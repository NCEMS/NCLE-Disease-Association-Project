...
# output file names
(fileName = paste("a1_1_type_", paste(types, collapse = "_and_"), "_pthr_70_", option, sep=""))
...
for (type in types) {
  ...
  dfs <- dataProcessing_df(type, 70) # function for data frame extraction
  ...
  UniScoDis <- final_dfA[, c("uniprotids","Entanglement", "Cov_Entanglement", "Knot", "50th_percentile", "75th_percentile", "95th_percentile")]
  ...
  final_df <- UniScoDis %>%
    group_by(uniprotids) %>%
    summarise(
      Entanglement      = ifelse(any(Entanglement == "Yes"), "Yes", "No"),
      Cov_Entanglement  = ifelse(any(Cov_Entanglement == "Yes"), "Yes", "No"),
      Knot              = ifelse(any(Knot == "Yes"), "Yes", "No"),
  ...
  ## --- clean up: keep only results, types, option (and the loop var can be dropped) ---
  keep_vars <- c("results", "fisher_results", "types", "option", "fileName", "final_df_af", "final_dfB_af", "final_df_crystal", "final_dfB_crystal")
