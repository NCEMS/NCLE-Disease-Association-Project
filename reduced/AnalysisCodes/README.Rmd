...
All scripts except `analysis_mut_all_cond_q1`, `analysis_mut_all_cond_q3`, `rf_and_match.R` and `sim_mis_prop` must be run after sourcing their corresponding control file, which defines global parameters (e.g., types, option, cov_vec, disease_only). See the [Execution README](Execute/README.html) for a detailed explanation of these parameters.
...
- Fits logistic regression models for for each disease class status vs Entanglement for Noncovalent Entanglements
  with scale(Length) + Essential as covariates.
- Applies BH correction to p-values.
- Produces an OR plot for a filtered subset (AF + 50th) and saves plot values.
...
- Via `Execute/RunAll.R` (recommended), which runs `analysis_mut_all_cond_q1.R`.
...
- Via `Execute/RunAll.R` (recommended), which runs `analysis_mut_all_cond_q3.R`.
...
- Plot (SVG/PDF):
  ...
  - Results/Plots/sim_mis_prop_type_af_pthr_70_<timestamp>_plot.pdf
