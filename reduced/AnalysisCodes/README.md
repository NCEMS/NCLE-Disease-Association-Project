...
All scripts except `analysis_mut_all_cond_q1`,
`analysis_mut_all_cond_q3`, `rf_and_match.R` and `sim_mis_prop` must be
run after sourcing their corresponding control file, which defines
global parameters (e.g., types, option, cov_vec, disease_only). See the
[Execution README](Execute/README.html) for a detailed explanation of
these parameters.
...
- Results tables:
  - `Results/Dataframes/a1_1_type_<types>_pthr_70_<option>_<timestamp>.csv`
  - `Results/Dataframes/fisher_a1_1_type_<types>_pthr_70_<option>_<timestamp>.csv`
...
- Fits logistic regression models for for each disease class status vs
  Entanglement for Noncovalent Entanglements with scale(Length) +
  Essential as covariates.
- Applies BH correction to p-values.
- Produces an OR plot for a filtered subset (AF + 50th) and saves plot
  values.
...
- Results table:
  - `Results/Dataframes/a1_2_type_<types>_pthr_70_<option>_<timestamp>.csv`
- Plots (SVG/PDF) + plotted values:
  - `Results/Plots/a1_2_af_noncov_50th_<timestamp>_plot.svg`
  - `Results/Plots/a1_2_af_noncov_50th_<timestamp>_plot.pdf`
...
- Results table:
  - `Results/Dataframes/a1_3_type_<types>_pthr_70_<timestamp>.csv`
- 50% plot values:
  - `Results/Dataframes/a1_3_type_<types>_pthr_70_<timestamp>_plot_values.csv`
- OR plot (SVG/PDF):
  - `Results/Plots/a1_3_type_<types>_pthr_70_<timestamp>_plot.svg`
  - `Results/Plots/a1_3_type_<types>_pthr_70_<timestamp>_plot.pdf`
...
- Results table:
  - `Results/Dataframes/mutation_q1_alltypes_pthr_70_results_len_res_<timestamp>.csv`

### How it is run

- Via `Execute/RunAll.R` (recommended), which runs
  `analysis_mut_all_cond_q1.R`.
...
- Results table:
  - `Results/Dataframes/mutation_q2_type_af_pthr_70_results_len_<timestamp>.csv`
...
- OR plot (SVG/PDF):
  ...
  - `Results/Plots/mut_q3_af_noncov_50th_<timestamp>_plot.pdf`

### How it is run

- Via `Execute/RunAll.R` (recommended), which runs
  `analysis_mut_all_cond_q3.R`.
...
- Plot (SVG/PDF):
  ...
  - Results/Plots/sim_mis_prop_type_af_pthr_70\_<timestamp>\_plot.pdf
