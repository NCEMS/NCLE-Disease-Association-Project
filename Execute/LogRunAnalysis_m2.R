...
log_file <- paste0("Log/ScriptOutput_af_plddt_", plddt_thresh, "_a_m2", format(Sys.time(), "_%Y-%m-%d_%H-%M-%S"), ".log")

# Redirect console output and messages to the log file
sink(log_file)

# Source the R script, logging all output
source("AnalysisCodes/analysis_mut_all_cond_q2.R", echo = T, max.deparse.length = 100000)
