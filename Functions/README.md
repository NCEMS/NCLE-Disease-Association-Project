...
These outputs are used as the primary inputs for downstream analyses
throughout the repository.

## `dataProcessing_df_analysis_3.R`

This script derives analysis-specific objects by defining percentile
thresholds from DisGeNET association scores, collapsing disease evidence
to the maximum score per protein, and joining that outcome information
to protein-level entanglement features. It further restricts the
resulting dataset to entangled proteins and computes derived
entanglement metrics by transforming $G_n$ and $G_c$ to absolute values
and creating $G_{\mathrm{sum}} = |G_n| + |G_c|$ and
$G_{\mathrm{max}} = \max\!\left(|G_n|, |G_c|\right)$.
...
- percentiles — A vector containing the 95th, 75th, and 50th percentile
  thresholds of DisGeNET association scores.

- final_df_updated — A protein-level dataset restricted to entangled
  proteins, containing:
...
This dataset is used for downstream statistical analyses examining
relationships between entanglement strength and disease association.

## `dataProcessing_df_mutation.R`

This script prepares entanglement region data for residue-level mutation
analyses by cleaning and filtering AF entanglement data and converting
entangled-region strings into numeric residue positions. The resulting
table can then be joined to mutation data to determine whether each
mutation falls within an entangled region.
...
- af_ent_dat_clean A protein-level dataset containing:
  - uniprotids — protein identifier
  - numeric_EntRegion — numeric vector of residue indices belonging to
    entangled regions
